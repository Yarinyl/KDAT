...
class RoIHeads(nn.Module):
    ...
    def forward(
        ...
        if self.training:
            ...
            proposals_with_labels = self.prob_vec_to_box(proposals, class_logits)
        ...
        return result, losses, proposals_with_labels

    def prob_vec_to_box(self, proposals, class_logits):
        pred_scores = F.softmax(class_logits/self.temperature, -1)
        max_prob_indices = torch.argmax(pred_scores, dim=1)
        boxes = proposals[0]
        inds = torch.where(max_prob_indices != 0)
        boxes, scores = boxes[inds], pred_scores[inds]
        return boxes, scores
